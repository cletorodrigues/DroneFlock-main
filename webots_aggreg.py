...
class CrazyflieDrone:
    ...
    def init_devices(self):
        ...
        with open('data_table.csv', 'w', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow(['Timestamp', 'Average Distance to Centroid'])  # Write header
    ...
    def aggregate(self):
        ...
        if self.motion_mode == 0:
            self.aggreg_mode = 0
            if self.my_id == 0:
                print("SELECT AGGREGATION MODE: \n '1' FOR LIN ALT AND BOUNDED REPULSION \n '2' FOR LIN BOUND BELLOW ATT AND UNREP \n '3' FOR ALM CONST ATT AND UNBOUND REP")
            ...
            if self.aggreg_mode == 1:
                ...
                print("LINEAR ATTRACTION AND BOUNDED REPULSION CASE \n PARAMETERS: a = ", self.a, " || b = ", self.b, " || c = ", self.c)
        ...
        self.write_in_file(self.robot.getTime(), avg_dist_to_centroid)
    ...
    def write_in_file(self, timestamp, avg_dist_2_centroid):
        # Write timestamp and avg_dist_to_centroid to CSV
        with open('data_table.csv', 'a', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow([timestamp, avg_dist_2_centroid])
